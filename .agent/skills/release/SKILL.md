...
### Bước 4: Soạn thảo Release Notes (Changelog)
Tự động phân tích sâu, tổng hợp và nhóm các thay đổi thành một chuỗi văn bản Markdown chất lượng cao.
**Yêu cầu bắt buộc**: Nội dung Release Notes phải nêu bật được **Bản chất nghiệp vụ / Tính năng cốt lõi** đã thay đổi, TUYỆT ĐỐI KHÔNG copy y nguyên một cách máy móc các log commit từ git.
- Hãy suy luận từ các commit để gom nhóm các thay đổi giống nhau hoặc phục vụ chung một mục đích thành 1 gạch đầu dòng duy nhất mang tính tóm tắt nghiệp vụ (Ví dụ: thay vì xả ra 5 commit đều có tên "update style", hãy gộp thành "Tinh chỉnh lại UI/UX cho màn hình ABC").
- Lược bỏ những câu commit không mang ý nghĩa rõ ràng đối với ngữ cảnh End-User.

Ví dụ Format đầu ra:
```markdown
## What's Changed
### 🚀 Features / Enhancements
- **QA Automation**: Triển khai hoàn thiện AI Skill tự động đọc spec và sinh Test Matrix.
- **Workflow**: Bổ sung tính năng sinh Release tự động và tính toán Semantic Versioning.
### 🐛 Bug Fixes
- **UI**: Khắc phục hiện tượng ẩn form khi mở tính năng tạo mới backlog (fix màn hình nhỏ).
### 🛠 Chores & Refactoring
- Cập nhật lại cấu trúc `README.md` đồng bộ hóa mô hình thiết kế của AI Agent.
```
